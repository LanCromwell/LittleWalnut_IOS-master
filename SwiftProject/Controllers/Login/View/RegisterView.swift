...
class RegisterView: UIView {
    ...
    ///邀请码输入框
    lazy var invitedTF: UITextField = {
        let inputTF = UITextField()
        inputTF.textColor = Color_019
        
        if (UIDevice.current.systemVersion as NSString).floatValue >= 13.0 {
            inputTF.attributedPlaceholder = NSAttributedString.init(string: "邀请码（选填）", attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 15),NSAttributedString.Key.foregroundColor : Color_100])
...
extension RegisterView {
    ...
    ///协议
    @objc fileprivate func scannProtocolEvent() {
        print("跳转web")
    }
...
extension RegisterView {
    ...
    fileprivate func loadUI() {
        ...
        scrollView.addSubviews([backButton, titleLab, phoneTF, phoneUnderLine, codeTF, codeBtn, codeUnderLine, pwdTF, pwdUnderLine, registerBtn, invitedTF, invitedUnderLine])
    ...
    fileprivate func loadFrame() {
        ...
        invitedTF.snp.makeConstraints { (make) in
            make.left.equalTo(35 * kWIDTHBASE)
            make.centerX.equalTo(scrollView)
            make.height.equalTo(48 * kWIDTHBASE)
            make.top.equalTo(pwdUnderLine.snp.bottom).offset(15 * kWIDTHBASE)
        }

        invitedUnderLine.snp.makeConstraints { (make) in
            make.left.right.equalTo(phoneTF)
            make.height.equalTo(0.5)
            make.top.equalTo(invitedTF.snp.bottom)
        }
        ...
        registerBtn.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(invitedUnderLine.snp.bottom).offset(50 * kWIDTHBASE)
