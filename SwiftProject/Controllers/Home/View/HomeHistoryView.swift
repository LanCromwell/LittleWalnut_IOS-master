...
class HomeHistoryView: UIView  {
    ...
    lazy var lineLable: UILabel = {
        ...
        lab.backgroundColor = UIColor.hexInt(999999)
           return lab
       }()
...
//            btn.addTarget(self, action: #, for: .touchUpInside)
//            return btn
//        }()
...
//        }()
        ...
        fileprivate lazy var collectionView: UICollectionView = {
            ...
            let collectionView = UICollectionView(frame: CGRect.init(x: 2, y: 2, width: kScreenWidth, height: kScreenHeight - 43*kWIDTHBASE), collectionViewLayout: layout)
...
extension HomeHistoryView {
      ...
      func loadFrame() {
        ...
        lineLable.snp.makeConstraints { (make) in
            make.top.equalTo(self.closedBtn.snp.top)
          make.left.right.equalTo(self)
            make.height.equalTo(1)
        }
        
        collectionView.snp.makeConstraints { (make) in
                   make.bottom.equalTo(self.closedBtn.snp.top)
                 make.left.right.equalTo(self)
                   make.height.equalTo(440*kWIDTHBASE)
               }
      }
  ...
  //         DFPlayer.shareInstance()?.category = .playback
      ...
      @objc fileprivate func backEvent() {
         DFPlayer.shared().df_deallocPlayer()
//        if self.myBackFuncBlock != nil{
//            myBackFuncBlock()
//        }
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "HistortyClosedNoty"), object: nil)
      ...
      ///获取 列表数据
       func obtainListData() {
          ...
          let params = ["page": page, "page_size": page_size, "user_id": userInfo?.id ?? "0"] as [String : Any]
          homeProvider.request(.userHistoryNoteListAPI(params: params)) { [weak self](result) in
              switch result {
...
//                        self.makeToast(model.msg, position: self?.center)
      ...
      fileprivate func loadMore() {
          page += 1
          let userInfo = UserInfo.default.findUserInfo()
          let params = ["page": page, "page_size": page_size, "user_id": userInfo?.id ?? "0", "category_id": 7] as [String : Any]
          homeProvider.request(.userCollectListAPI(params: params)) { [weak self](result) in
              switch result {
              case .success(let res):
                  let jsonStr = String.init(data: res.data, encoding: .utf8)
                  try? print("result.mapJSON() = \(res.mapJSON())")
...
extension HomeHistoryView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension HomeHistoryView: DZNEmptyDataSetSource, DZNEmptyDataSetDelegate {
...
extension HomeHistoryView: DFPlayerDataSource, DFPlayerDelegate {
