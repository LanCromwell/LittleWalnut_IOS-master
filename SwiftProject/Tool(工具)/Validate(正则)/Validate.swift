...
struct Validate {
    ...
    static func checkMobile(_ mobileNumbel: String) -> Bool {
        //去除空格
        let phoneStr = mobileNumbel.removeAllSapce
        ...
        if regextestmobile.evaluate(with: phoneStr) || regextestcm.evaluate(with: phoneStr) || regextestcu.evaluate(with: phoneStr) || regextestct.evaluate(with: phoneStr) {
            return true
        }
        return false
    }
    ...
    //MARK:- 正则匹配手机号
    static func feCheckMobile(_ mobileNumbel: String) -> Bool {
        //去除空格
        let phoneStr = mobileNumbel.removeAllSapce
        let MOBIL = "1([0-9]){10}"
        let regextestmobile = NSPredicate(format: "SELF MATCHES %@", MOBIL)
        if regextestmobile.evaluate(with: phoneStr) {
            return true
        }
        return false
    }
